...
public class SmtpWorker {
			...
			if (!user.isEmpty()) {
				if (!pass.isEmpty()) {
					if (resp.matches("(?s).*AUTH.*PLAIN.*")) {
						...
						String s2 = "Authentication is required but the server did not support it.";
	...
	private void onCommandMailFrom(Rfc822Token[] tokens) throws IOException,
			MessengerException {
	...
	public void sendMessage(MessageForSend syncMessage)
		...
		Rfc822Token[] cc = Rfc822Tokenizer.tokenize(message.getCarbonCopy());
